...
- Optionally uses Claude to **auto-categorize** conversations into projects you define, with confidence scoring and interactive review for uncertain assignments
- Runs locally — categorization uses the Claude CLI, everything else has zero network calls
...
If you have the Claude CLI installed, you can auto-categorize conversations into projects:
...
This sends conversation titles and content snippets to Claude, which assigns each conversation to a project (or leaves it unassigned if it doesn't fit). The tool runs two passes — a second pass re-evaluates any low-confidence assignments using descriptions it builds from the first pass results.
...
- **Categorize sends conversation titles and short text snippets to Claude** via the CLI for classification. No full conversations are sent — just enough context to determine which project a conversation belongs to.
